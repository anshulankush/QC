...
public class HealthBusiness extends ListActivity {
	...
	private class GetContacts extends AsyncTask<Void, Void, Void> {
		...
		protected void onPreExecute() {
			...
			pDialog = new ProgressDialog(HealthBusiness.this);
		...
		protected Void doInBackground(Void... arg0) {
			...
			try {
				...
				for (int i = 0; i < businessJsonArray.length(); i++) {
					...
					if(group.equals("Health & Fitness")){
		...
		protected void onPostExecute(Void result) {
			...
			adapter = new SimpleAdapter(
					HealthBusiness.this, businessList,
			...
			filterEditText.addTextChangedListener(new TextWatcher() {
				...
				public void onTextChanged(CharSequence s, int start, int before, int count) {
					...
					for (HashMap<String, String> t: businessList)
					{
						...
						adapter = new SimpleAdapter(
								HealthBusiness.this, searchResults2,
								R.layout.list_row_allbusiness, new String[] { TAG_BUSINESS  }, new int[] { R.id.business});
						setListAdapter(adapter);
					}
				}
